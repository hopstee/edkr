...
export default async function AboutView() {
    ...
    const personalInfoFile = await fs.readFile(process.cwd() + `/data/${locale}/personal.json`, "utf-8");
    const personalInfo = JSON.parse(personalInfoFile);

    const contactsFile = await fs.readFile(process.cwd() + `/data/contacts.json`, "utf-8");
    const contacts = JSON.parse(contactsFile);

    const skillsFile = await fs.readFile(process.cwd() + `/data/${locale}/skills.json`, "utf-8");
